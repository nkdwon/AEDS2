...
class SelectionSort{
    ...
    // MÉTODO PARA ORDERNAR PARCIALMENTE OS POKÉMONS ATRAVÉS DO MÉTODO SELECTION SORT
    void ordernarPorSelecaoParcial(ArrayList<Pokemon> pokedex, Contador contador, int k) {
        for (int i = 0; i < k; i++) { // Loop para percorrer os primeiros k elementos da lista
            int menor = i; // Inicializa o índice do menor elemento como o índice atual
            for (int j = i + 1; j < pokedex.size(); j++) {
                contador.comparacoes++;
                // Compara o nome do Pokémon no índice 'menor' com o nome do Pokémon no índice 'j'
                if ((pokedex.get(menor).getName().compareTo(pokedex.get(j).getName()) > 0)) {
                    menor = j; // Atualiza o índice do menor elemento se um elemento menor for encontrado
                }
            }
            // Verifica se o menor elemento encontrado é diferente do elemento atual 
            // Para evitar swaps desnecessários
            if (menor != i) {
                contador.swaps++;
                swap(pokedex, i, menor);
            }
        }
    }
}
...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        timeLog.start();
        selection.ordernarPorSelecaoParcial(pokedex, contador, 10);
        timeLog.end();

        // Chama o método para imprimir os Pokémons ordenados
        imprimirPokemonsOrdenados(pokedex, 10);
        ...
        try (FileWriter logWriter = new FileWriter(matricula + "_selecao_parcial.txt")) {
