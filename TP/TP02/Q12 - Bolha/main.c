...
void bubbleSortId(Pokemon pokedex[], int size) {
    for (int i = 0; i < size - 1; i++) {
        for (int j = 0; j < size - i - 1; j++) {
            ...
            pokedex[j].id == pokedex[j + 1].id && (strcmp(pokedex[j].name, pokedex[j+1].name) > 0)) {
                (countSwaps)++; // Contar troca
                swap(&pokedex[j], &pokedex[j + 1]); // Troca
            } 
        }
    }
}
...
int main()
{
    ...
    start = clock();
    bubbleSortId(pokedex, numPokemonsPokedex);
    ...
    FILE *logFile = fopen("732683_bolha.txt", "w");
    if (logFile == NULL) {
