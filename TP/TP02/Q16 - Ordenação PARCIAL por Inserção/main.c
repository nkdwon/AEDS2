...
// Função para converter a string de data no formato "dd/mm/yyyy" para struct tm
void stringParaData(const char *strData, struct tm *dataTm) {
    // Limpa a estrutura tm
    memset(dataTm, 0, sizeof(struct tm));

    // Converte a string de data para struct tm
    sscanf(strData, "%d/%d/%d", &dataTm->tm_mday, &dataTm->tm_mon, &dataTm->tm_year);

    // Ajusta o mês e o ano, já que o tm_mon conta de 0 a 11 e o tm_year começa em 1900
    dataTm->tm_mon -= 1;        // tm_mon vai de 0 (janeiro) a 11 (dezembro)
    dataTm->tm_year -= 1900;    // Ajusta o ano para o formato esperado por tm
}


// Método para comparar dois Pokémon
int compare(Pokemon *a, Pokemon *b){
    struct tm dataA, dataB;

    // Converte as strings de data para struct tm
    stringParaData(a->data, &dataA);
    stringParaData(b->data, &dataB);

    // Usa mktime para converter as structs tm para time_t e fazer a comparação
    time_t timeA = mktime(&dataA);
    time_t timeB = mktime(&dataB);

    // Compara as datas (difftime retorna positivo, negativo ou zero)
    if (timeA != timeB) {
        return (timeA > timeB) ? 1 : -1;
    }
    // Se as datas forem iguais, compara pelos nomes
    return strcmp(a->name, b->name);
}

// MÉTODO PARA ORDENAR OS POKEMON POR CAPTURE DATE COM INSERTION SORT PARCIAL
void ordernarPorInsercaoParcial(Pokemon pokedex[], int numPokemonsPokedex, int k) {
    // Itera a partir do segundo elemento até o final da lista (considera que o primeiro já está ordenado)
    for (int i = 1; i < numPokemonsPokedex; i++){
        // Armazena o Pokémon atual a ser posicionado
        Pokemon tmp = pokedex[i];
        int j = (i < k) ? i - 1 : k - 1; // Define o índice de comparação
    
        // Compara o pokémon atual com os anteriores e se menor, move pra esquerda
        while (j >= 0 && (compare(&pokedex[j], &tmp) > 0)){
            countComps++;
            pokedex[j+1] = pokedex[j]; // Move o Pokémon maior para a direita
            countSwaps++;
            j--; // Decrementa o índice para comparar com o próximo anterior
        }
        countSwaps++;
        pokedex[j + 1] = tmp; // Insere o Pokémon atual na posição correta
    }
}
...
int main()
{
    ...
    start = clock();
    ordernarPorInsercaoParcial(pokedex, numPokemonsPokedex, k);
    for(int i = 0; i < k; i++){
    ...
    FILE *logFile = fopen("732683_insercao_parcial.txt", "w");
    if (logFile == NULL) {
