...
class InsertionSort {
    ...
    public int compare(Pokemon a, Pokemon b){
        // Compara as datas de captura
        int dateCmp = a.getCaptureDate().compareTo(b.getCaptureDate());
        if(dateCmp != 0){
            return dateCmp; // Se as datas forem diferentes, retorna a comparação
        }
    ...
    // MÉTODO PARA ORDENAR
    public void ordenarPorCaptureDate(ArrayList<Pokemon> pokedex, Contador contador) {
        ...
        for (int i = 1; i < pokedex.size(); i++){
            ...
            while (j >= 0 && (compare(pokedex.get(j), tmp) > 0)){
                contador.comparacoes++;
                pokedex.set(j + 1, pokedex.get(j)); // Move o Pokémon maior para a direita e o atual para a esquerda
                contador.swaps++;
                j--; // Decrementa o índice para comparar com o próximo anterior
            }
            contador.swaps++;
            pokedex.set(j + 1, tmp); // Insere o Pokémon atual na posição correta
        }
    }
}
...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        timeLog.start();
        insertion.ordenarPorCaptureDate(pokedex, contador);
        timeLog.end();
        ...
        try (FileWriter logWriter = new FileWriter(matricula + "_insercao.txt")) {
