...
class Quicksort{
    ...
    /** CONSTRUTOR
     * Inicializa o objeto Quicksort com a lista de Pokémon, contador e o limite de ordenação parcial
     * @param pokedex  ArrayList de Pokémon a ser ordenada
     * @param contador Objeto para contabilizar o número de comparações e swaps
     * @param k        Limite de elementos a serem ordenados parcialmente
     */
    public Quicksort(ArrayList<Pokemon> pokedex, Contador contador, int k) {
    ...
    /** MÉTODO SWAP
    * @param pokedex ArrayList de Pokémon
    * @param i      índice i (primeiro Pokémon)
    * @param j      índice j (segundo Pokémon)
    */
    static void swap(ArrayList<Pokemon> pokedex, int i, int j) {
        Pokemon tmp = pokedex.get(i);
        pokedex.set(i, pokedex.get(j));
        pokedex.set(j, tmp);
    }

    /** MÉTODO PARA COMPARAR OS NOMES DOS POKÉMONS
     * @param p1 Pokémon 1 a ser comparado
     * @param p2 Pokémon 2 a ser comparado
     * @return valor negativo se p1 < p2, positivo se p1 > p2, 0 se forem iguais
     */
    public int nameCompare(Pokemon p1, Pokemon p2) {
        return p1.getName().compareTo(p2.getName());
    }

    /** MÉTODO PARA ORDERNAR PARCIALMENTE OS POKÉMONS ATRAVÉS DO MÉTODO QUICKSORT
    * @param esq índice esquerdo do intervalo a ser ordenado
    * @param dir índice direito do intervalo a ser ordenado
    */
    void ordenarPorQuickParcial(int esq, int dir) {
        int i = esq, j = dir;
        Pokemon pokemonPivo = pokedex.get((esq + dir) / 2); // Escolhe o pivô como o Pokémon do meio

        // Particiona a lista em relação ao pivô
        while (i <= j) {
    
            // Move o índice i para a direita enquanto o Pokémon atual for menor que o pivô
            while (pokedex.get(i).getGeneration() < pokemonPivo.getGeneration() || 
                (pokedex.get(i).getGeneration() == pokemonPivo.getGeneration() && nameCompare(pokedex.get(i), pokemonPivo) < 0)) {
                contador.comparacoes++;
                i++;
            }
            
            // Move o índice j para a esquerda enquanto o Pokémon atual for maior que o pivô
            while (pokedex.get(j).getGeneration() > pokemonPivo.getGeneration() || 
                (pokedex.get(j).getGeneration() == pokemonPivo.getGeneration() && nameCompare(pokedex.get(j), pokemonPivo) > 0)) {
                contador.comparacoes++;
                j--;
            }

             // Se i ainda não passou j, faz a troca entre pokedex[i] e pokedex[j]
            if (i <= j) {
                contador.swaps++;
                swap(pokedex, i, j); // Troca
                i++;
                j--;
            }
        }
        ...
        // Chamada recursiva para ordenar o lado esquerdo da partição
        if (esq < j) {
            ordenarPorQuickParcial(esq, j);
        }
        // Chamada recursiva para ordenar o lado direito da partição, mas limitando até 'k' elementos (ordenação parcial)
        if (i < k && i < dir) {
            ordenarPorQuickParcial(i, dir);
        }
    }
}
...
public class Main {
    ...
    static void imprimirPokemonsOrdenados(ArrayList<Pokemon> pokedex, int k) {
        for (int i = 0; i < k; i++) {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Quicksort quick = new Quicksort(pokedex, contador, k);

        // Mede o tempo de execução ao ordenar
        timeLog.start();
        quick.ordenarPorQuickParcial(0, pokedex.size() - 1);
        timeLog.end();

        // Chama o método para imprimir os Pokémons ordenados
        imprimirPokemonsOrdenados(pokedex, k);
        ...
        try (FileWriter logWriter = new FileWriter(matricula + "_quicksort_parcial.txt")) {
