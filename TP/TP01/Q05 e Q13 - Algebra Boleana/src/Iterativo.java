...
public class Iterativo {
    ...
    public static char resolverExpressao(String novaExpressao) {
        ...
        int i = 0;
        while (i < novaExpressao.length()) { // Percorre cada caractere da expressão
            char x = novaExpressao.charAt(i); // Lê o caractere atual
            if (x != ')') { // Se não for um parêntese de fechamento
                pilha.adicionar(novaExpressao.charAt(i)); // Adiciona o caractere na pilha
            } else { // Se for um parêntese de fechamento
                String subExpressao = ""; // Inicializa a sub-expressão
                do {
                    subExpressao = pilha.remover() + subExpressao; // Remove elementos até encontrar um '('
                } while (subExpressao.charAt(0) != '(');
