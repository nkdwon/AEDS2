...
class ArvoreBinariaPrimaria {
    ...
    public boolean pesquisar(Pokemon p) throws Exception
    {
        NoPrimario aux = this.raiz;
        int modCapRate = p.getCaptureRate() % 15;

        while(aux != null)
        {
            if (aux.cptRate > modCapRate)
            {
                aux = aux.esq;
            }
            else if (aux.cptRate < modCapRate)
            {
                aux = aux.dir;
            }
            else
            {
                aux.arvoreSecundaria.inserir(p);
                return true;
            }
        }
        return false;
    ...
    public void caminharPos(ArvoreBinariaPrimaria pokedexArvoreBinariaPrimaria, String nome)
    {
        boolean[] aux = new boolean[1];
        aux[0] = false;
        caminharPosRec(pokedexArvoreBinariaPrimaria.raiz, nome, true, aux);

        if (aux[0] == true) System.out.println("  SIM");
        else System.out.println("  NAO");
    }

    private void caminharPosRec(NoPrimario node, String nome, boolean x, boolean[] aux)
    {
        if (node != null && aux[0] == false)
        {
            aux[0] = node.arvoreSecundaria.pesquisar(nome);
            
            if (aux[0] == false) System.out.print("  ESQ");
            caminharPosRec(node.esq, nome, true, aux);
            if (aux[0] == false) System.out.print("  DIR");
            caminharPosRec(node.dir, nome, false, aux);
	...
	private NoPrimario inserir(int x, NoPrimario i) throws Exception
    {
		if (i == null)
        {
            i = new NoPrimario(x);
...
class ArvoreBinaria
{
    ...
    public boolean pesquisar(String nome)
    {
        ...
        while(aux != null)
        {
            if (aux.pokemon.getName().compareTo(nome) > 0)
            {
                aux = aux.esq;
                System.out.print(" esq");
            }
            else if (aux.pokemon.getName().compareTo(nome) < 0)
            {
                aux = aux.dir;
...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String entrada;
        ArvoreBinariaPrimaria pokedexCaptureRate = new ArvoreBinariaPrimaria();

        //7, 3, 11, 1, 5, 9, 13, 0, 2, 4, 6, 8, 10, 12 e 14
        pokedexCaptureRate.inserir(7);
        pokedexCaptureRate.inserir(3);
        pokedexCaptureRate.inserir(11);
        pokedexCaptureRate.inserir(1);
        pokedexCaptureRate.inserir(5);
        pokedexCaptureRate.inserir(9);
        pokedexCaptureRate.inserir(13);
        pokedexCaptureRate.inserir(0);
        pokedexCaptureRate.inserir(2);
        pokedexCaptureRate.inserir(4);
        pokedexCaptureRate.inserir(6);
        pokedexCaptureRate.inserir(8);
        pokedexCaptureRate.inserir(10);
        pokedexCaptureRate.inserir(12);
        pokedexCaptureRate.inserir(14);
            ...
            if (entrada.matches("\\d+")) { // Verifica se a string contém apenas dígitos
                try {
                    ...
                    if (pokemonEncontrado != null) {
                        pokedexCaptureRate.pesquisar(pokemonEncontrado);
                    } else {
        ...
        while(!entrada.equals("FIM"))
        {
            ...
            pokedexCaptureRate.caminharPos(pokedexCaptureRate, entrada);
            entrada = scanner.nextLine();
        }
        ...
        try (FileWriter logWriter = new FileWriter(matricula + "_arvoreArvore.txt")) {
